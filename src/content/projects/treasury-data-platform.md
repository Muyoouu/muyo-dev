...
The product takes that data in through pipelines, cleans and structures it, and serves it back through a web dashboard, so the people responsible for the numbers can see where things stand. The product itself isn't public, so this datasheet covers how it's built instead of what it looks like.

I've owned most of the technical execution since the first commit, including the early calls on the data model, the service layout, and the deployment.
...
Everything runs in Docker containers on Azure. Containerizing from the start kept deploys predictable and made moving between environments straightforward. When something breaks, the container setup makes it clear where the problem lives.
