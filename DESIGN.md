...
- **SpecCard** (`SpecCard.astro`) — the package-flip card, the signature component. Outer `.package` gives `perspective: 1400px`. Front face: `border-2 border-foreground bg-card`, a muted spec bar (`Component` mono-label left, mono date right), title link (red on group hover), 15px description, mono-label bordered tags. Rear face: stacked in the same grid cell (`grid-area: 1 / 1`, `backface-visibility: hidden`), `rotateY(180deg)`, `border-2 border-accent`, red SPEC bar with white mono text, `divide-y` dl (ROLE and TAGS as `dt` mono-label over `dd` value rows; TAGS as `flex-wrap` mono spans separated by `·`; the array holds tool names and keywords, while discipline lives in `role`, which the same face already shows), footer with filled "Full datasheet" + up to two hairline links (Source/Demo/Article). `pointer-events: none` on the hidden face so it cannot intercept taps meant for the visible one. See Motion for gating.
